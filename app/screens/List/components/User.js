...
export default class User extends Component {
  ...
  handleBuySoda(e){//Husk setstate oppdaterer neste render
    ...
    console.log(`User ${this.state.name} bought a soda for ${SODA_PRICE},- new balace ${newBalance}`);
  }
  handleDeposit(e){
    e.preventDefault()
    ...
    this.setState({balance:newBalance});
    console.log(`User ${this.state.name} deposited ${this.depositAmount.value}, new balance: ${newBalance}`);//TODO: logger feil balanse
  ...
  render() {
    return (
      <tr>
        ...
        <td><button className="btnWithraw" onClick={this.handleBuySoda}>Buy soda</button></td>
        <td>
          <form role="form" onSubmit={this.handleDeposit}>
            <input type="number" ref={ref => (this.depositAmount = ref)} className="inpDeposit"/>
